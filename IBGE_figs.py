...
trace_male = go.Bar(
    ...
    name='Homens',
    hoverinfo='text',
    text=df_dist_idade_sexo["Quantidade"][df_dist_idade_sexo["Sexo"] == "Homens"],
...
trace_female = go.Bar(
    ...
    name='Mulheres',
    hoverinfo='text',
    text=df_dist_idade_sexo["Quantidade"][df_dist_idade_sexo["Sexo"] == "Mulheres"],
...
sun_pop_fig = px.sunburst(
    ...
    color_discrete_sequence=["#967f51", "#E6D1B1","#1f1e1c", "#f7cd79", "#454128"],
    width=800,
    height=800
)

sun_pop_fig.update_traces(
    hovertemplate="<br>".join([
        "População: %{value}"
    ])
)
